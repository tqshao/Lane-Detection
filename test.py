...
def draw_lines(img, lines, color=[0, 255, 0], thickness=10):
	...
	for line in lines:
		...
		if abs(slope) > slope_threshold and abs(slope)!=999:
			slopes.append(slope)
...
def annotate_image_array(image_in):
	...
	return lines,image,gray,blur_gray,edges,initial_image,annotated_image

def annotate_image(input_file, output_file):
	""" Given input_file image, save annotated image to output_file """
	lines,image,gray,blur_gray,edges,initial_image,annotated_image = annotate_image_array(mpimg.imread(input_file))
...
input_file = "./test_images/NEW4.jpg"
output_file = "NEW.jpg"

lines,image,gray,blur_gray,edges,initial_image,annotated_image = annotate_image(input_file, output_file)
...
fig.set_size_inches(int(size[0]*4), int(size[1])*4)
...
#    fig = plt.gcf()
#    size = fig.get_size_inches()
#    fig.set_size_inches(int(size[0]*2), int(size[1]*2))
#    plt.plot([x1,x2],[y1,y2],'r',linewidth=3.0)
